...
function checkfield($fields, $_MyPOST)
{
 ...
 */
function createAuthCode($string, $operation = 'DECODE', $key = '', $expiry = 0)
{
    $ckey_length = 4;
    // 随机密钥长度 取值 0-32;
...
function substr250($str)
{
    return mb_strwidth($str, 'UTF8') > 250 ? mb_substr($str, 0, 250, 'utf-8') . '...' : $str;
}

function substr20($str)
{
    return mb_strwidth($str, 'UTF8') > 20 ? mb_substr($str, 0, 20, 'utf-8') . '...' : $str;
}
...
function idcard_verify_number($idcard_base)
{
    ...
    for ($i = 0; $i < strlen($idcard_base); $i++) {
        $checksum = $checksum + (integer)substr($idcard_base, $i, 1) * $factor[$i];
    }
    $mod = $checksum % 11;
